...
class VideoReceiver:
    def __init__(self, host='localhost', port=9999):
        ...
        self.last_nsfw_check = 0
        self.nsfw_check_interval = 0.0  # Check every frame (no interval)
    ...
    def check_nsfw(self, frame):
        """Check if frame contains NSFW content"""
        if not self.nsfw_detection_available:
            return False, 0.0
        
        try:
            self.detection_count += 1
            logger.info(f"Running NSFW detection (attempt #{self.detection_count})...")
            ...
            for result in results:
                logger.info(f"Detection result: {result['label']} - Confidence: {result['score']:.3f}")
            ...
            if nsfw_confidence > sfw_confidence:
                logger.warning(f"NSFW content detected! Confidence: {nsfw_confidence:.3f}")
                return True, nsfw_confidence
            else:
                logger.info(f"SFW content detected. Confidence: {sfw_confidence:.3f}")
            ...
            while True:
                ...
                try:
                    ...
                    if frame is not None:
                        ...
                        # Check for NSFW content on every frame
                        if self.nsfw_detection_available:
                        ...
                        if self.gun_detection_available and (self.frame_count_since_last_gun_check >= self.gun_detection_interval):
                            ...
                            if self.gun_detection_available:
                                ...
                                if self.current_gun_status:
                                    print(f"[CACHED] WEAPON detected! Confidence: {self.gun_confidence:.2f}")
                                else:
                                    print(f"[CACHED] No weapons - Confidence: {self.gun_confidence:.2f}")
                        ...
                        if self.gun_detection_available:
                            # Show when the result is cached vs. fresh
                            cache_indicator = "" if self.frame_count_since_last_gun_check == 0 else f"[CACHED {self.frame_count_since_last_gun_check}/{self.gun_detection_interval}]"
                            
                            if self.current_gun_status:
                                cv2.putText(frame, f"WEAPON: {self.gun_confidence:.2f} {cache_indicator}", (10, y_pos), 
                                           cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
                                cv2.putText(frame, f"Objects: {len(self.gun_objects)}", (10, y_pos+30), 
                                           cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
                                cv2.putText(frame, f"Next check in: {self.gun_detection_interval - self.frame_count_since_last_gun_check} frames", (10, y_pos+60), 
                                           cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 2)
                            else:
                                cv2.putText(frame, f"No weapons detected {cache_indicator}", (10, y_pos), 
                                           cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
                                cv2.putText(frame, f"Next check in: {self.gun_detection_interval - self.frame_count_since_last_gun_check} frames", (10, y_pos+30), 
